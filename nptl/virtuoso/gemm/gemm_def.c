...
#include <sys/syscall.h>
#include <limits.h>
// #include <struct pthread.h>
#include <virtuoso/pthread_types.h>
#include <virtuoso/vam/vam_physical_accel.h>
#include <virtuoso/gemm/gemm_params.h>
...
#include <virtuoso/esp/esp_accelerator.h>
// #include <nn_token.h>
...
void *gemm_invoke(void *a) {
    ...
    bool *kill_pthread = &args->kill_pthread;
    bitmap_t *valid_contexts_ack = &args->valid_contexts_ack;
    uint64_t *context_runtime = args->active_cycles; // for VAM
    ...
    while (1) {
        ...
        if (bitmap_none(accel->valid_contexts)) {
            SCHED_YIELD;
            continue;
        }
        ...
        for (int i = 0; i < MAX_CONTEXTS; i++) {
            if (!bitmap_test(accel->valid_contexts, new_context)) {
                ...
                } else if (bitmap_test(accel->valid_contexts, new_context) && context_vruntime[new_context] < min_vruntime) {
        ...
        HIGH_DEBUG(
            if (bitmap_count(accel->valid_contexts) > 1)
        ...
        for (int i = 0; i < MAX_CONTEXTS; i++) {
            if (bitmap_test(accel->valid_contexts, i) && !bitmap_test(*valid_contexts_ack, i)) {
                ...
                HIGH_DEBUG(printf("[INVOKE] Added context %d on %s for struct pthread %s\n", i, accel->devname, struct pthread_get_name(th[i]));)
            }
        }
...
#ifndef ENABLE_VAM
void gemm_init(struct physical_accel_t *accel, void *mem) {
    struct gemm_stratus_access *gemm_access_desc;
    gemm_access_desc = (struct gemm_stratus_access *) malloc (sizeof(struct gemm_stratus_access));
    enum contig_alloc_policy policy;
...
void gemm_run(struct physical_accel_t *accel, gemm_queue_entry_t *e) {
    gemm_params_t *params = &(e->gemm_params);
